...
import sys, random, pyperclip, cryptomath
SYMBOLS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890 !?.'

def main():
    my_message = """"A computer would deserve to be called intelligent if \
        it could deceive a human into believing that it was human." \
            - Alan Turing"""
    my_key = 2894
    my_mode = 'encrypt'     # encrypt or decrypt
    print(my_message)
...
def check_keys(key_a, key_b, mode):
 ...
 0 and %s.' % (len(SYMBOLS) - 1))
    if cryptomath.gcd(key_a, len(SYMBOLS) - 1):
        sys.exit('\nKey A (%s) and the symbol set size (%s) are not relatively\
 prime. Choose a different key.\n' % (key_a, len(SYMBOLS)))
...
def decrypt_message(key, message):
    ...
    for symbol in message:
        if symbol in SYMBOLS:
            # Decrypt the symbol:
            symbol_index = SYMBOLS.find(symbol)
            plaintext += SYMBOLS[(symbol_index - key_b) * mod_inverse_of_key_a \
                % len(SYMBOLS)]
        else:
